...
const SettingClassModal = () => {
  const { isOpen, modalType, onClose, data } = useModalStore();
  const isModalOpen = isOpen && modalType === "settingClass";
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.patch(`/api/classes/${classData?.id}`, values);
      ...
      <DialogContent className="bg-white text-black p-0 overflow-hidden">
        ...
        <Form {...form}>
          <form className="space-y-8" onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <DialogFooter className="bg-gray-100 px-6 py-4 flex w-full ">
              <Button
                variant="destructive"
                disabled={
...
export default SettingClassModal;
